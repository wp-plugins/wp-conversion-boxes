...
class Pardot_OM_API {
	...
	function get_lists( $args = array() ) {
		$campaigns = false;
		if ( $response = $this->get_response( 'list', $args ) ) {
			$campaigns = array();
            if ( $response->result->total_results >= 200 ) {
                $limit = 200;
            } else {
            ...
            for( $i = 0; $i < $limit; $i++ ) {
                $campaign = (object)$response->result->list[$i];
                if ( isset($campaign->id) ) {
                    $campaigns[(int)$campaign->id] = $this->SimpleXMLElement_to_stdClass( $campaign );
                }
            }
            ...
            if ( $limit >= 200 ) {
	            $numpag = round($response->result->total_results/200)+1;
	            for( $j = 2; $j <= ($numpag); $j++ ) {
	                if ( $response = $this->get_response( 'list', $args, 'result', $j ) ) {
	                    for( $i = 0; $i < ($response->result->total_results-200); $i++ ) {
	                        $campaign = (object)$response->result->list[$i];
	                        if ( isset($campaign->id) ) {
	                            $campaigns[(int)$campaign->id] = $this->SimpleXMLElement_to_stdClass( $campaign );
...
x	 */
	...
	private function _get_url( $item_type, $args = array() ) {
		if ( 'login' == $item_type ) {
		    ...
		    if ( 'account' == $item_type )
		        $action = 'read';
            else if ( 'prospect' == $item_type )
                $action = 'create/email/' . $args['email'];
            else
                $action = 'query';
			$base_url = str_replace(
				array( '%%VERSION%%', '%%ITEM_TYPE%%', '%%ACTION%%' ),
				array( self::VERSION, $item_type, $action ),
