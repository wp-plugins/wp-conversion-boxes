<?php
/**
 * @package   WP_Conversion_Boxes_Pro
 * @author    Ram Shengale <admin@wpconversionboxes.com>
 * @license   GPL-2.0+
 * @copyright 2014 Ram Shengale
 *
 * @wordpress-plugin
 * Plugin Name:       WP Conversion Boxes Pro
 * Plugin URI:        http://wpconversionboxes.com
 * Description:       Same Traffic. More Conversions. Replace your CTAs and email/optin subscriber boxes with WP Conversion Boxes and skyrocket your conversion rate.
 * Version:           1.3.3
 * Author:            Ram Shengale
 * Author URI:        http://ramshengale.in
 * Text Domain:       wp-conversion-boxes-pro
 * Domain Path:       /lang/
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 */

if ( ! defined( 'WPINC' ) ) {
	die;
}

/***********************************************
 * Defines
 **********************************************/

define( 'WPCB_CURRENT_VERSION', '1.3.3' );

define( 'WPCB_WEBSITE_URL', 'http://wpconversionboxes.com' );
define( 'WPCB_PRODUCT_NAME', 'WP Conversion Boxes Pro' );

define('ADMIN_ASSETS_URL', plugins_url('admin/assets/', __FILE__ ));
define('PUBLIC_ASSETS_URL', plugins_url('public/assets/', __FILE__ ));

define('WPCB_TEMPLATE_DIR_PATH', plugin_dir_path(__FILE__).'templates/');
define('WPCB_TEMPLATE_DIR_URL', plugins_url( 'templates/',__FILE__));

$wpcb_upload_dir = wp_upload_dir();
define('WPCB_CUSTOM_TEMPLATE_DIR_PATH', $wpcb_upload_dir['basedir'] . '/wpcb-custom-templates/' );
define('WPCB_CUSTOM_TEMPLATE_DIR_URL', $wpcb_upload_dir['baseurl'] . '/wpcb-custom-templates/' );

define('MAILERS_DIR_PATH', plugin_dir_path(__FILE__).'admin/mailers/');



/***********************************************
 * Text Localization
 **********************************************/

function wpcb_init(){
    load_plugin_textdomain('wp-conversion-boxes-pro', false, dirname(plugin_basename(__FILE__)) . "lang/");
}
add_action('init', 'wpcb_init');


/***********************************************
 * Include and initialize public facing classes.
 **********************************************/

require_once( plugin_dir_path( __FILE__ ) . 'public/class.tracker.php' );
require_once( plugin_dir_path( __FILE__ ) . 'public/class.public.php' );
add_action( 'plugins_loaded', array( 'WPCB_Tracker', 'get_instance' ) );
add_action( 'plugins_loaded', array( 'WPCB_Public', 'get_instance' ) );



/***********************************************
* Plugin activate and deactivate hook.
***********************************************/

register_activation_hook( __FILE__, array( 'WPCB_Public', 'activate' ) );
register_deactivation_hook( __FILE__, array( 'WPCB_Public', 'deactivate' ) );



/**********************************************
 * Include and initialize admin side classes
 **********************************************/

if ( is_admin() ) {
    
        // Include all admin classes
        
	require_once( plugin_dir_path( __FILE__ ) . 'admin/class.admin.php' );
        require_once( plugin_dir_path( __FILE__ ) . 'admin/classes/class.metabox.php' );
        require_once( plugin_dir_path( __FILE__ ) . 'admin/classes/class.licensing.php' );
        
        // Initialize all admin classes
        add_action( 'plugins_loaded', array( 'WPCB_Admin', 'get_instance' ) );
        add_action( 'plugins_loaded', array( 'WPCB_Metabox', 'get_instance' ) );
        add_action( 'plugins_loaded', array( 'WPCB_Licensing', 'get_instance' ) );

}